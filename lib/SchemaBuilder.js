...
SchemaBuilder.prototype._resolverForModel = function (modelData, extraQuery) {
  ...
  return function (knex, ignore1, ignore2, data) {
    ...
    var eager = self._buildEager(ast, modelClass, data.fragments);
 ...
 * @private
 */
SchemaBuilder.prototype._buildEager = function (astNode, modelClass, fragments) {
  var filters = {};
  ...
  for (var i = 0, l = astNode.selectionSet.selections.length; i < l; ++i) {
    ...
    if (relation) {
      ...
      var subExpr = this._buildEager(selection, relation.relatedModelClass, fragments);
      ...
      var fragmentSelection = fragments[selection.name.value];
      var fragmentExpr = this._buildEager(fragmentSelection, modelClass, fragments);
