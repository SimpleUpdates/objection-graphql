...
SchemaBuilder.prototype._relationField = function (modelData, relation) {
  if (relation instanceof objection.HasManyRelation || relation instanceof objection.ManyToManyRelation) {
    ...
    var isRequired = _.intersection(relation.ownerModelClass.jsonSchema.required, relation.ownerProp).length === relation.ownerProp.length;
    var type = this._typeForModel(modelData);
    if(isRequired) {
...
SchemaBuilder.prototype._resolverForModel = function (modelData, extraQuery) {
  ...
  return function (knex, ignore1, ignore2, data) {
    ...
    var eager = self._buildEager(ast, modelClass, data);
    var filter = self._filterForArgs(ast.arguments, modelClass, data.variableValues);
 ...
 * @private
 */
SchemaBuilder.prototype._buildEager = function (astNode, modelClass, astRoot) {
  var filters = {};
  ...
  for (var i = 0, l = astNode.selectionSet.selections.length; i < l; ++i) {
    ...
    if (relation) {
      ...
      if (selection.arguments.length) {
        ...
        var filter = this._filterForArgs(selection.arguments, relation.relatedModelClass, astRoot.variableValues);
      ...
      var subExpr = this._buildEager(selection, relation.relatedModelClass, astRoot);
      ...
      var fragmentSelection = astRoot.fragments[selection.name.value];
      ...
      if (fragmentExpr.expression.length) {
        ...
        _.assign(filters, fragmentExpr.filters);
      }
...
SchemaBuilder.prototype._filterForArgs = function (args, modelClass, variableValues) {
  if (args.length === 0) {
    return null;
  }
