...
class SchemaBuilder {
  ...
  _relationField(modelData, relation) {
    if (relation instanceof objection.HasOneRelation 
      ...
      const isRequired = _.intersection(relation.ownerModelClass.jsonSchema.required, relation.ownerProp.cols).length === relation.ownerProp.cols.length;
  ...
  _resolverForModel(modelData, extraQuery) {
    return (ctx, ignore1, ignore2, data) => {
      ...
      const selectsForVirtual = {};
      const addSelects = (astNode, graphQLType) => {
        const fields = graphQLType.getFields();
        if(!selectsForVirtual[graphQLType.name]) {
          selectsForVirtual[graphQLType.name] = [];
        }
        for (let i = 0, l = astNode.selectionSet.selections.length; i < l; ++i) {
          let selectionNode = astNode.selectionSet.selections[i];
          let nodeName = selectionNode.name.value;
          if(selectionNode.kind === 'FragmentSpread') {
            selectionNode = data.fragments[nodeName];
            addSelects(selectionNode, graphQLType);
            continue;
          }
          const field = fields[nodeName];
          if(!field) {
            continue;
          }
          ...
          if (fieldType.constructor.name === 'GraphQLObjectType') {
            addSelects(selectionNode, fieldType);
          } else if(nodeName == 'edges') {
            // Hack/shortcut support for Relay connections
            selectionNode = selectionNode.selectionSet.selections[0]
            addSelects(selectionNode, fieldType.ofType.getFields().node.type);
          } else if (fieldType.constructor.name === 'GraphQLList') {
            addSelects(selectionNode, fieldType.ofType);
          } else {
            selectsForVirtual[graphQLType.name].push(nodeName);
          }
        }
      }
      const objectType = data.returnType.ofType || data.returnType;
      addSelects(ast, objectType);
  ...
  _buildEager(astNode, modelClass, astRoot) {
    ...
    for (let i = 0, l = astNode.selectionSet.selections.length; i < l; ++i) {
      ...
      if (relation) {
        ...
        var fragmentSelection = astRoot.fragments[selectionNode.name.value];
        var fragmentExpr = this._buildEager(fragmentSelection, modelClass, astRoot);
        ...
        ++numExpressions;
      } else if(selectionNode.name.value == 'edges') {
        //hack support for Relay connections
        const result = this._buildEager(selectionNode.selectionSet.selections[0], modelClass, astRoot);
        ({expression, filters} = result);
      }
    }
  ...
  _filterForArgs(astNode, modelClass, variableValues) {
    ...
    if (args.length === 0 || this.models === undefined) {
      return null;
    }
    ...
    for (let i = 0, l = args.length; i < l; ++i) {
      ...
      // Special case for stringified-ids
    ...
    return (builder) => {
      for (var i = 0, l = argObjects.length; i < l; ++i) {
        ...
        if(!modelData.args[arg.name] || modelClass.virtualAttributes && modelClass.virtualAttributes.includes(arg.name)) {
          continue;
        }
  ...
  _filterForSelects(astNode, modelClass, astRoot) {
    /* Disabling the select filtering until we can figure out how to select virtual attributes. */
    return null;
...
function toJson(result) {
  if(result && result.results) {
    result.results = formatResults(result.results);
    return result;
