...
const _ = require('lodash')
  ...
  , GraphQLID = new graphqlRoot.GraphQLNonNull(graphqlRoot.GraphQLID);
...
function jsonSchemaToGraphQLFields(jsonSchema, opt) {
  ...
  _.forOwn(jsonSchema.properties, (propSchema, propName) => {
    ...
    fields[propName] = toGraphQLField(propSchema, propName, ctx, jsonSchema.required.indexOf(propName) !== -1);
  });
...
function toGraphQLField(jsonSchema, propName, ctx, required) {
  ...
  if (jsonSchema.anyOf || jsonSchema.oneOf) {
    ...
    if (schemas.length === 1) {
      return toGraphQLField(schemas[0], propName, ctx, required);
    } else {
...
function typeToGraphQLField(type, jsonSchema, propName, ctx, required) {
  ...
  if (_.isArray(jsonSchema.enum)) {
    ...
    graphQlField = primitiveToGraphQLField('id');
  } else {
...
function primitiveToGraphQLField(type, required) {
  ...
  graphQlType = required ? new graphqlRoot.GraphQLNonNull(graphQlType) : graphQlType;
